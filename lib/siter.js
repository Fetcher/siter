...
function siter(url, callbacks) {
  ...
  // Twitter Picture
  if (require('./sites/picTwitter.js')(url, callbacks)) return;

  // 9gag! Picture
  if (require('./sites/9gag.js')(url, callbacks)) return;

  // Instagram! Picture
  if (require('./sites/instagram.js')(url, callbacks)) return;

  // Facebook! Picture
  if (require('./sites/facebook.js')(url, callbacks)) return;

  // Google! Picture
  if (require('./sites/google.js')(url, callbacks)) return;

  // Google Plus+! Picture
  if (require('./sites/googleplus.js')(url, callbacks)) return;

  // Pinterest! Picture
  if (require('./sites/pinterest.js')(url, callbacks)) return;

  // Flickr Picture
  if (require('./sites/flickr.js')(url, callbacks)) return;
  
  // Deviantart Picture
  if (require('./sites/deviantart.js')(url, callbacks)) return;

  // YouTube
  if (require('./sites/youtube.js')(url, callbacks)) return;
  
  // Vimeo
  if (require('./sites/vimeo.js')(url, callbacks)) return;

  // Dailymotion
  if (require('./sites/dailymotion.js')(url, callbacks)) return;

  // Vine.co
  if (require('./sites/vine.js')(url, callbacks)) return;  
